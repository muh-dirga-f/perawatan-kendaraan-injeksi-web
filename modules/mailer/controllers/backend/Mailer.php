...
class Mailer extends Admin	
{
	...
	*/
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => 'administrator/mailer/index/',
			...
			if ($save_mailer) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/mailer/edit/' . $save_mailer, 'Edit Mailer'),
						anchor('administrator/mailer', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/mailer');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
