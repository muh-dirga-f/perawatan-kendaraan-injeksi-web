...
class Email_template extends Admin	
{
	...
	*/
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => 'administrator/email_template/index/',
			...
			if ($save_email_template) {
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/email_template/edit/' . $save_email_template, 'Edit Email Template'),
						anchor('administrator/email_template', ' Go back to list')
					]);
				} else {
					set_message(
            		...
            		$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/email_template');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
