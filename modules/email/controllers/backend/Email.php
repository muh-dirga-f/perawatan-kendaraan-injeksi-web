...
class Email extends Admin
{
	...
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => 'administrator/email/index/',
			...
			if ($save_email) {
				...
				if ($this->input->post('save_type') == 'stay') {
					...
					$this->data['message'] = cclang('success_save_data_stay', [
						anchor('administrator/email/edit/' . $save_email, 'Edit Email'),
						anchor('administrator/email', ' Go back to list')
					]);
				} else {
					set_message(
					...
					$this->data['success'] = true;
					$this->data['redirect'] = base_url('administrator/email');
				}
			} else {
				if ($this->input->post('save_type') == 'stay') {
